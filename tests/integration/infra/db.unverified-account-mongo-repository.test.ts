...
function makeAccountToken(
  accountToken: string = "encripted_account_id",
): string {
  return accountToken;
}
...
describe("Add Unverified Account Mongo Repository", () => {
  ...
  afterEach(async () => {
    unverifiedAccountCollection = await mongo.getCollection(
      "unverifiedAccounts",
    );
  ...
  test("Should return unverifiedAccount", async () => {
    const sut = makeSut();
    const unverifiedAccount = await sut.add(
      makeAccountToken("some-random-token"),
    );
  ...
  test("Should insert unverifiedAccount corretly in the database", async () => {
    ...
    const accountMongo = await unverifiedAccountCollection.findOne({
  ...
  test("Should delete unverifiedAccount corretly from the database", async () => {
    ...
    let accountMongo = await unverifiedAccountCollection.findOne({
      accountToken,
    });
    expect(accountMongo).toBeTruthy();
